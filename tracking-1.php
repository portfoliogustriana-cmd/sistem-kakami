...
<head>
    ...
    <script>
        // 2. Cek Tema saat halaman pertama dimuat (Mencegah layar berkedip putih/FOUC)
...
<body class="text-slate-800 dark:text-slate-200 bg-slate-50 dark:bg-slate-900 antialiased transition-colors duration-300">
    ...
    <div class="p-4 sm:p-8 w-full">
        <div class="max-w-7xl mx-auto space-y-8">
            ...
            <!-- HEADER BERGAYA GLASSMORPHISM DENGAN DUKUNGAN DARK MODE -->
            <div class="flex flex-col md:flex-row justify-between items-center bg-white/60 dark:bg-slate-800/60 backdrop-blur-md p-6 rounded-3xl shadow-sm border border-slate-200/60 dark:border-slate-700/60">
                <div>
                    ...
                    <h2 class="text-slate-900 dark:text-white font-black">Tracking Antrean</h2>
                ...
                <div class="mt-4 md:mt-0 flex items-center gap-3">
                    <form method="GET" class="flex items-center gap-3 bg-white dark:bg-slate-800 p-2 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                        <label class="font-bold text-xs text-slate-600 dark:text-slate-300 pl-2">Filter Bulan:</label>
                        <input type="month" name="bulan" value="<?php echo $bulan_filter; ?>" class="border-none bg-slate-50 dark:bg-slate-900/80 text-slate-800 dark:text-slate-100 p-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900 dark:focus:ring-slate-500 text-xs font-semibold cursor-pointer transition">
                        <button type="submit" class="bg-gradient-to-r from-slate-900 to-slate-700 dark:from-slate-700 dark:to-slate-600 text-white px-5 py-2 rounded-xl font-bold hover:from-black hover:to-slate-800 dark:hover:from-slate-600 dark:hover:to-slate-500 transform hover:scale-105 active:scale-95 transition-all shadow-lg shadow-slate-900/20 text-xs text-center border border-transparent dark:border-slate-500/30">Cari</button>
                    </form>
                    
                    <!-- TOMBOL TOGGLE DARK MODE -->
                    <button onclick="toggleTheme()" class="bg-white dark:bg-slate-800 p-3 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-yellow-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
                        <!-- Icon Bulan (Tampil di tampilan Terang) -->
                        ...
                        <!-- Icon Matahari (Tampil di tampilan Gelap) -->
                        <svg class="w-5 h-5 hidden dark:block" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4.22 1.336a1 1 0 011.415 0l.707.707a1 1 0 01-1.414 1.414l-.707-.707a1 1 0 010-1.414zM18 10a1 1 0 01-1 1h-1a1 1 0 110-2h1a1 1 0 011 1zM15.536 14.12aa1 1 0 010 1.415l-.707.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM10 16a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zm-4.22-1.336a1 1 0 01-1.415 0l-.707-.707a1 1 0 111.414-1.414l.707.707a1 1 0 010 1.414zM4 10a1 1 0 01-1 1H2a1 1 0 110-2h1a1 1 0 011 1zM5.88 4.75a1 1 0 010-1.414l-.707-.707a1 1 0 111.414-1.414l.707.707a1 1 0 01-1.414 1.414zM10 6a4 4 0 100 8 4 4 0 000-8z" clip-rule="evenodd"></path></svg>
                    </button>
                </div>
            </div>

            <!-- KOTAK INDIKATOR DENGAN EFEK HOVER MELAYANG (ADAPTIF DARK MODE) -->
            <div>
                <div class="flex justify-between items-end mb-4 px-2">
                    ...
                    <?php if($status_aktif !== ''): ?>
                        <a href="tracking.php?bulan=<?php echo $bulan_filter; ?>" class="text-[10px] bg-red-50 dark:bg-rose-900/30 text-red-600 dark:text-rose-400 font-bold px-4 py-1.5 rounded-full border border-red-200 dark:border-rose-800/50 hover:bg-red-100 dark:hover:bg-rose-900/50 transition-all uppercase tracking-wider shadow-sm"><svg class="w-[1em] h-[1em] mb-0.5 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"></path></svg> Reset Filter</a>
                    <?php endif; ?>
                </div>
                <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                    <?php 
                    $menu_status = [
                        ['id' => 'DP', 'icon' => '<svg class="w-[1em] h-[1em] mb-0.5 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path></svg>', 'name' => 'DP / Masuk', 'bg' => 'from-slate-50 to-slate-100 dark:from-slate-800/80 dark:to-slate-700/80', 'border' => 'border-slate-200 dark:border-slate-600', 'text' => 'text-slate-800 dark:text-slate-200'],
                        ['id' => 'Produksi', 'icon' => '<svg class="w-[1em] h-[1em] mb-0.5 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M6 9V2h12v7M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 012 2v5a2 2 0 01-2 2h-2m-4-7v6H8v-6h8z"></path></svg>', 'name' => 'Produksi', 'bg' => 'from-blue-50 to-indigo-50 dark:from-blue-900/30 dark:to-indigo-900/30', 'border' => 'border-blue-200 dark:border-blue-800/50', 'text' => 'text-blue-800 dark:text-blue-400'],
                        ['id' => 'Jait', 'icon' => '<svg class="w-[1em] h-[1em] mb-0.5 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg>', 'name' => 'Jait', 'bg' => 'from-purple-50 to-fuchsia-50 dark:from-purple-900/30 dark:to-fuchsia-900/30', 'border' => 'border-purple-200 dark:border-purple-800/50', 'text' => 'text-purple-800 dark:text-purple-400'],
                        ['id' => 'Checking', 'icon' => '<svg class="w-[1em] h-[1em] mb-0.5 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>', 'name' => 'Checking', 'bg' => 'from-amber-50 to-orange-50 dark:from-amber-900/30 dark:to-orange-900/30', 'border' => 'border-amber-200 dark:border-amber-800/50', 'text' => 'text-orange-800 dark:text-orange-400'],
                        ['id' => 'Selesai', 'icon' => '<svg class="w-[1em] h-[1em] inline-block mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"></path></svg>', 'name' => 'Selesai', 'bg' => 'from-emerald-50 to-teal-50 dark:from-emerald-900/30 dark:to-teal-900/30', 'border' => 'border-emerald-200 dark:border-emerald-800/50', 'text' => 'text-emerald-800 dark:text-emerald-400']
                    ];
                    foreach($menu_status as $m): 
                        $active = ($status_aktif == $m['id']);
                        $active_class = $active ? 'ring-2 ring-offset-2 dark:ring-offset-slate-900 ring-slate-800 dark:ring-slate-300 scale-105 shadow-md' : 'hover:-translate-y-1 hover:shadow-lg opacity-80 hover:opacity-100';
                    ?>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700/60">
                            ...
                            <?php else: ?>
                                <?php while($row = mysqli_fetch_assoc($query_tracking)): 
                                    ...
                                    if ($stat == 'Produksi') $grad_color = 'from-blue-400 to-indigo-500 shadow-blue-500/50 dark:shadow-blue-900/50';
                                    if ($stat == 'Jait') $grad_color = 'from-purple-400 to-fuchsia-500 shadow-purple-500/50 dark:shadow-purple-900/50';
                                    if ($stat == 'Checking') $grad_color = 'from-amber-400 to-orange-500 shadow-orange-500/50 dark:shadow-orange-900/50';
                                    if ($stat == 'Selesai') $grad_color = 'from-emerald-400 to-teal-500 shadow-teal-500/50 dark:shadow-teal-900/50';

                                    // MODERN SLA BADGES
                                    $selisih = (strtotime($row['deadline_kerja'] ?? $row['tanggal_order']) - strtotime(date('Y-m-d'))) / 86400;
                                    
                                    if ($stat == 'Selesai') {
                                        $badge_html = "<span class='bg-emerald-50 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/60 px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-widest shadow-sm'><svg class="w-[1em] h-[1em] inline-block mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"></path></svg> Tuntas</span>";
                                    } elseif ($selisih < 0) {
                                        ...
                                        $badge_html = "<span class='bg-slate-100 dark:bg-slate-700/60 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600/60 px-3 py-1 rounded-full text-[10px] font-bold tracking-wide'>Aman ($selisih hari)</span>";
                                    }
                                ?>
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors duration-200 text-center group">
                                    
                                    <!-- INFO PROJECT -->
                                    <td class="p-5 text-left border-r border-slate-100 dark:border-slate-700/60 align-top">
                                        <span class="inline-block bg-slate-100 dark:bg-slate-700/80 text-slate-500 dark:text-slate-300 px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider mb-1"><?php echo htmlspecialchars($row['invoice_no']); ?></span>
                                        <span class="font-extrabold text-slate-900 dark:text-slate-100 block text-sm uppercase group-hover:text-slate-700 dark:group-hover:text-slate-300 transition"><?php echo htmlspecialchars($row['nama_order']); ?></span>
                                        <div class="flex items-center gap-1 mt-1 text-[11px] text-slate-500 dark:text-slate-400 font-semibold">
                                            <div class="w-4 h-4 bg-slate-200 dark:bg-slate-600 rounded-full flex items-center justify-center text-[8px]"><svg class="w-[1em] h-[1em] mb-0.5 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 11c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"></path></svg></div>
                                            <?php echo htmlspecialchars($row['nama_konsumen']); ?>
                                        </div>
                                        <a href="cetak_spk.php?id=<?php echo $row['id']; ?>" target="_blank" class="inline-flex items-center gap-1 mt-3 bg-white dark:bg-slate-700/50 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 font-bold px-3 py-1.5 rounded-lg text-[10px] uppercase hover:bg-slate-800 dark:hover:bg-slate-600 hover:text-white dark:hover:text-white hover:border-slate-800 dark:hover:border-slate-500 transition-all shadow-sm">
                                    ...
                                    <!-- TARGET & SLA -->
                                    <td class="p-5 border-r border-slate-100 dark:border-slate-700/60 align-top">
                                        <div class="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-3 border border-slate-100 dark:border-slate-700/60 inline-block text-center w-full">
                                            <div class="font-black text-xl text-slate-800 dark:text-slate-100 leading-none"><?php echo (int)$row['jumlah_pcs']; ?> <span class="text-xs font-semibold text-slate-400 dark:text-slate-500">Pcs</span></div>
                                            <div class="w-full h-px bg-slate-200 dark:bg-slate-700/60 my-2"></div>
                                    ...
                                    <!-- PROGRESS BAR -->
                                    <td class="p-5 border-r border-slate-100 dark:border-slate-700/60 align-middle">
                                    ...
                                    <td class="p-5 align-top">
                                        <form method="POST" class="flex flex-col gap-2 w-full max-w-[380px] mx-auto text-left">
                                            ...
                                            <select name="status_tracking" class="w-full bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-600/80 text-slate-700 dark:text-slate-200 p-2.5 rounded-xl text-[11px] font-extrabold uppercase outline-none focus:bg-white dark:focus:bg-slate-800 focus:ring-2 focus:ring-slate-800 dark:focus:ring-slate-400 transition-all cursor-pointer shadow-sm">
                                            ...
                                            <textarea name="kendala_produksi" rows="3" class="w-full bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-600/80 p-3 rounded-xl text-[11px] outline-none focus:bg-white dark:focus:bg-slate-800 focus:ring-2 focus:ring-slate-800 dark:focus:ring-slate-400 transition-all shadow-sm resize-none placeholder-slate-400 dark:placeholder-slate-500 text-slate-600 dark:text-slate-200" placeholder="Ketik kendala di sini..."><?php echo htmlspecialchars($row['kendala_produksi'] ?? ''); ?></textarea>
                                            
                                            <button type="submit" class="w-full bg-gradient-to-r from-slate-800 to-slate-900 dark:from-slate-700 dark:to-slate-800 text-white py-2.5 rounded-xl text-[10px] font-extrabold uppercase tracking-widest hover:from-black hover:to-slate-800 dark:hover:from-slate-600 dark:hover:to-slate-700 transform hover:-translate-y-0.5 active:translate-y-0 transition-all shadow-lg shadow-slate-900/20 dark:shadow-black/40 flex justify-center items-center gap-2 border border-transparent dark:border-slate-600/50">
