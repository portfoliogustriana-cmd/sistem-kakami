...
<body class="text-slate-800 dark:text-slate-200 bg-slate-50 dark:bg-slate-900 antialiased transition-colors duration-300 flex flex-col min-h-screen">
    ...
    <div class="p-4 sm:p-8 w-full">
        <div class="max-w-7xl mx-auto space-y-6">
            ...
            <!-- LIVE SEARCH BAR -->
            <div class="bg-white dark:bg-slate-800 p-4 rounded-3xl border border-slate-100 dark:border-slate-700/60 flex items-center gap-3.5 shadow-sm">
                <span class="text-slate-400 dark:text-slate-500 pl-1"><svg class="w-4 h-4 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg></span>
                <input id="searchInput" type="text" onkeyup="filterKonsumen()" placeholder="Cek cepat nama konsumen atau nomor handphone..." class="w-full bg-transparent border-none text-xs outline-none focus:ring-0 font-bold placeholder-slate-400 dark:placeholder-slate-500 text-slate-800 dark:text-slate-200">
            </div>
                    ...
                    <table class="w-full text-sm text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors duration-200 text-center group">
                                ...
                                <td class="p-4 text-left font-extrabold text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors uppercase text-xs">
                                    <a href="detail_konsumen.php?nama=<?php echo urlencode($r['nama_konsumen']); ?>" class="hover:underline">
                                        <?php echo htmlspecialchars($r['nama_konsumen']); ?>
                                    </a>
                                </td>
    ...
    function filterKonsumen() {
        ...
        rows.forEach(row => {
            ...
            if (cols.length < 3) return; // skip indicator rows or small rows
            
            const nama = cols[1].textContent.toLowerCase();
            const telp = cols[2].textContent.toLowerCase();
